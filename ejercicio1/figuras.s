...
//x9 = color del cuadrado

ldr x24, =letra_O          // x24 = dirección de la letra (letra_O)
mov x6, 0                  // contador de fila

bucle_filas:
    cmp x6, 5
    bge fin_letra

    // Cargar el valor de la fila de la matriz
    lsl x7, x6, #3            // offset = fila * 8
    add x8, x24, x7           // dirección de la fila
    ldr x10, [x8]              // cargar la fila

    mov x5, 0                  // contador columna

bucle_columnas:
    cmp x5, 5
    bge sig_fila

    // Extraer bit correspondiente a la columna
    mov x11, x10           // copiar fila actual
    lsr x11, x11, x5           // desplazar columna bits a la derecha
    and x11, x11, 1             // aislar bit

    cbz x11, no_dibujar

// Calcular posición de cada cuadrado
    mov x19, x3                  // x base
    mov x20, x2                  // y base
    mov x21, x5
    lsl x21, x21, #2              // columna * 4
    add x21, x21, x19              // x = base + col * 4

    mov x22, x6
    lsl x22, x22, #2              // fila * 4
    add x22, x22, x20              // y = base + fila * 4
